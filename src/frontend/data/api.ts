...
export const api = {
  events: {
    ...
    packingCategories: {
      create: (eventId: string, body: { name: string }) =>
        unwrap(
          client.api.events({ id: eventId }).packing_categories.post(body),
        ),
      update: (eventId: string, cid: string, body: { name?: string }) =>
        unwrap(
          client.api
            .events({ id: eventId })
            .packing_categories({ cid })
            .put(body),
        ),
    ...
    packingItems: {
      ...
      update: (
        ...
        unwrap(
          client.api.events({ id: eventId }).packing_items({ pid }).put(body),
        ),
      delete: (eventId: string, pid: string) =>
        unwrap(
          client.api.events({ id: eventId }).packing_items({ pid }).delete(),
        ),
    },
    volunteers: {
  ...
  budgets: {
    ...
    updateLineItem: (
      ...
      unwrap(
        client.api.budgets({ id: budgetId }).line_items({ itemId }).put(body),
      ),
    deleteLineItem: (budgetId: string, itemId: string) =>
      unwrap(
        client.api.budgets({ id: budgetId }).line_items({ itemId }).delete(),
      ),
  },
  ...
  members: {
    ...
    create: (body: Record<string, unknown>) =>
      unwrap(client.api.members.post(body)),
    update: (id: string, body: Record<string, unknown>) =>
  ...
  contacts: {
    ...
    create: (body: Record<string, unknown>) =>
      unwrap(client.api.contacts.post(body)),
    update: (id: string, body: Record<string, unknown>) =>
  ...
  mailingLists: {
    list: async () => {
      const data = await unwrap(client.api.mailing_lists.get());
      return Array.isArray(data) ? data : [];
    },
    get: (id: string) => unwrap(client.api.mailing_lists({ id }).get()),
    create: (body: Record<string, unknown>) =>
      unwrap(client.api.mailing_lists.post(body)),
    update: (id: string, body: Record<string, unknown>) =>
      unwrap(client.api.mailing_lists({ id }).put(body)),
    delete: (id: string) => unwrap(client.api.mailing_lists({ id }).delete()),
    preview: (id: string) =>
      unwrap(client.api.mailing_lists({ id }).preview.get()),
    getMembers: (id: string) =>
      unwrap(client.api.mailing_lists({ id }).members.get()),
    addMember: (
      listId: string,
      contactId: string,
