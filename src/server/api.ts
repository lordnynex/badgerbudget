...
export const api = {
  ...
  seed: async () => {
    const db = getDb();
    const existing = db.query("SELECT COUNT(*) as c FROM budgets").get() as { c: number };
    if (existing.c > 0) return { ok: true, message: "Already seeded" };
    ...
    try {
      ...
      const items = raw.BadgerComparisonBudget?.line_items ?? [];
